...
function richtextCallback(x, data){
	selectCategoryDialog($(x).attr('data-url'),$(x).attr('data-publish'),data,$(x).attr('data-main'),$(x).attr('data-col'));
}
...
function selectCategoryDialog(url,pageType,type,pageTitle,publishCol)
{
	...
	if($('#manage_categories')[0] ==null)
	{
		...
		{
		var adc =$('<a>').appendTo(Ldiv).addClass('right').click(function(){addCategory()}).html('<i class="material-icons">add</i>').attr({"href":"javascript:;","id":"adc"});
		}
	...
	function nAccordion(prt,val,icon)
	{
			...
			//var add =$('<a>').appendTo(hdr).addClass('right_btn').html('<i class="material-icons small">add</i>').attr({'href':'javascript:;','id':val[1]});
