	...
	function loadFilter(a,p,npD,nc,callback)
	{
		...
		condition=cnd.join('|');
		$.getJSON("processAjax.php?pageType="+pagetype+"&p=0&l=4&condition="+condition, function(result){
	...
	function createFilterRow(p,npD,field,order,nc,callback)
	{
		...
		$('<input>').attr({'type':'checkbox','id':'filtercbx'+field.i}).val(nc+','+field.i).appendTo(nDiv);
