...
extract($_POST, EXTR_OVERWRITE);
	...
	if(isset($filter_checkbox)) $ids=str_replace("|",",",$filter_checkbox); else if(isset($sendId)) $ids=$sendId; else $ids="";
		...
		{
			$query="select group_concat(distinct($actionCol)) from $table where abs( $actionCol ) >0 and abs( $actionCol ) <>'80' ";
		} else
		{
			$query="select group_concat(distinct($actionCol)) from $table where $primary_key in ({$ids})";	
		}
		...
		if($rw=$rs->fetch_row())
		{
			...
			if(isset($k[0]) && strstr($k[0],"OK") !==false)
			{
				...
				$query="insert into sent_messages values('','SMS','$number','$message','$Generic_id',now())";
