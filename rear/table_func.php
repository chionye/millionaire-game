...
function getFields($t)
{
	...
	global $db;
	$result=$db->query("desc $t") or die($db->error);
...
function extractRow($table,$primary_key, $id)
{
	global $db;
	$result=$db->query("desc $table") or die($db->error);
	while($row=$result->fetch_row())
	{
	  $col_desc[]=strtolower($row[0])  ;
	}
...
function getLastInsert($primary_key,$table,$param)
{
	global $db;
	$query="select $param from $table order by $primary_key desc limit 1";
...
function getLastInsert2($primary_key,$table,$param)
{
	global $db;
	$query="select $param from $table order by $primary_key desc limit 1";
...
function getSynchParam($primary_key,$table,$param,$search)
{
	$query="select $param from $table where $primary_key='$search' ";
	$gt_result=$db->query($query) or die($db->error.$query);
...
function getParam($primary_key,$table,$param,$search)
{
	global $db;
	$query="select $param from $table where $primary_key='$search' limit 1";
	$gt_result=$db->query($query) or die($db->error.$query);
	if($r=$gt_result->fetch_row())
	{
		return $r[0];
...
function updateParam($primary_key,$table,$param,$search,$value)
{
	global $db;
	$query="update $table set $param ='$value' where $primary_key='$search' limit 1";
	$gt_result=$db->query($query) or die($db->error.$query);
	return $gt_result;
}
function incrementParam($primary_key,$table,$param,$search,$value)
{
	global $db;
	$q="select $param from $table where $primary_key='$search' limit 1";
	$rs=$db->query($q) or die($db->error.$query);
	if($rw=$rs->fetch_row())
	{
		$query="update $table set $param='' where $primary_key='$search' and $param is null limit 1";
		$gt_result=$db->query($query) or die($db->error.$query);
		$query="update $table set $param= $param + '$value' where $primary_key='$search' limit 1";
		$gt_result=$db->query($query) or die($db->error.$query);
	} else
	{
		$query="insert into $table set $param=  '$value', $primary_key='$search' , transcid=concat(unix_timestamp(),round((rand() * 100) * (rand() * 100) )) ";
		$gt_result=$db->query($query) or die($db->error.$query);
	}
	$query="select transcid from $table where $primary_key='$search' limit 1";
