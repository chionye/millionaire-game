<?php  $subType="services";
			require_once "select_subpage.php";
...
<div class="top-control row"> 
            ...
            <div class="input-field col s4"><input name="search" type="text"  id='_searchBox' size="50" value="" /><label for="_searchBox">Search <?php echo $sub_title ?></label></div><div class="col s2"><a href="javascript" class="btn ">Actions</a></div><div class="col s3 right"><a href="#_newForm_modal_<?php echo $subType; ?>_<?php echo str_replace(' ','_',$page_title); ?>" class="btn modal-trigger"><i class="large material-icons" id="new_<?php echo $sub_title; ?>">add</i>New</a></div></div>
...
<div class="modal modal-fixed-footer" id="_newForm_modal_<?php echo $subType; ?>" >

	 <form id="formData_<?php echo $subType; ?>"> <div class="modal-content black-text"><h4><?php echo "New ".$sub_title; ?></h4><?php foreach($sub_coldesc as $k => $v) { ?> <div class="row"><div class="col s12 "><div class="row" style="padding-bottom:20px;overflow:visible;">
  ...
  <?php foreach($pages as $k => $v ) { $vp=json_decode($v['props'],true);?>
    <li>
      ...
      <a href="#!" class=" modal-action modal-close waves-effect waves-green btn subformSave" data-action="#formData_<?php echo $subType; ?>_<?php echo str_replace(' ','_',$page_title); ?>" >SUBMIT</a><a href="#!" class=" modal-action modal-close waves-effect waves-green btn-flat">CLOSE</a>

    </div><input name="<?php echo $primary_key ?>" type="text" id="<?php echo $primary_key ?>" value="" style="display:none" class="uniqueId" />
