...
<table class="bordered">
      <thead>
        <th>&nbsp;</th>
        <?php for($j=0;$j<count($t[0]);$j++){if(!empty($t[0][$j]) && $j < 42){ ?>
		  <th <?php if($j > 2 && $j < 42)echo 'colspan="4"' ?>>
			<?php if($j != 0){ ?>
     ...
     <tr <?php if($i == 1){?>height="110"<?php }?>>
       <td>
        <?php if($i > 1){?> <input name="rcbox<?php echo $i?>" type="checkbox" id="checkbox<?php echo $i?>" title="uncheck to deselect" checked="checked" value="<?php echo $i?>" /><label for="checkbox<?php echo $i?>"></label><?php }?>
       </td>
        <?php for($j=0;$j<count($t[$i]);$j++){
			if($i == 0){
				if(!empty($t[$i][$j])){ ?><td <?php if($j > 2 && $j < 44)echo 'colspan="4"' ?>><?php echo $t[$i][$j] ?> </td> <?php }
			}else{?>
		 		<td><?php echo $t[$i][$j] ?> </td> 
		 <?php } } ?>
	  </tr> 
	  <?php } ?></tbody>   
    </table>
